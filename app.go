...
func main() {
	// setup tables
	db, err := genmai.New(&genmai.PostgresDialect{}, os.Getenv("DATABASE_URL"))
	if err != nil {
		log.Fatalln(err)
	}
	...
	authFile := os.Getenv("VADDY_AUTH_FILENAME")
	authCode := os.Getenv("VADDY_AUTH_CODE")
	goji.Get("/assets/*", http.FileServer(http.Dir(".")))
	goji.Get("/" + authFile, func(w http.ResponseWriter, r *http.Request) {fmt.Fprintf(w, authCode)})
	goji.Get("/", showPages)
